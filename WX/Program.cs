...
namespace WX
{
    class Program
    {
        ...
        static bool checkStringForUpperCaseOrNumber(string text, out string newText)
        {
            const int ignoraQuatroPrimeiros = 4;
            // purificar a string que entra e devolver apenas a mesma no formato METAR/TAF
            string metar = " ";
            for (int i = ignoraQuatroPrimeiros; i < text.Length; i++)
            {

                // Nao trazer as primeiras 4 ocorrencias de i pois traduz-se num codigo ICAO repetido

                if (char.IsUpper(text[i]) || char.IsDigit(text[i]))
                    metar = metar + text[i]; // concatenação de cada ocorrencia à variavel METAR
                if (char.IsWhiteSpace(text[i]))
                    metar = metar + " ";

                //Existem caracteres que não interessam
                if (i > 40)
                {
        ...
        /*
        static bool TransformaMetarEmXML(string [] array)
        {
